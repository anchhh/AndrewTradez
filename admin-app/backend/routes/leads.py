...
@bp.post("/<int:lead_id>/flow-result")
def save_flow_result(lead_id):
    """A finished Flow shot, coming back from the extension.

    The round trip closes here: Studio said what it wanted, Flow made it, and
    this files it against the side it was asked for so the drone step can fly
    from it. Sent inline as a data URL for the same reason listing photos
    are -- the extension has the bytes and no reason to stage them anywhere.
    """
    from services import dronepath
    from studio import save_capture_data_url
    ...
    path = dronepath.set_generated(lead, side, saved)
    db.session.commit()
    return jsonify({"side": side, "image": saved,
